...
class BaseDetector(nn.Module, ABC):
    ...
    def set_thresholds(self, tau_low: float, tau_high: float) -> None:
        """라운드마다 호출되어 pseudo-label 임계값을 갱신한다."""
        self.tau_low = tau_low
        self.tau_high = tau_high

    def set_pseudo_config(self, use_soft_pseudo: bool, soft_pseudo_weight: float) -> None:
        """pseudo-label 학습 동작을 갱신한다."""
        self.use_soft_pseudo = use_soft_pseudo
        self.soft_pseudo_weight = soft_pseudo_weight
...
class DummyDetector(BaseDetector):
    ...
    def unsupervised_loss(self, images, teacher):
        ...
        # soft 의사 라벨은 교사 confidence 평균으로 가중한다.
        if self.use_soft_pseudo and soft_mask.sum() > 0:
            soft_w = conf[soft_mask].mean()
            soft_w = self.soft_pseudo_weight * soft_w
