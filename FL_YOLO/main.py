...
class SyntheticUnlabeled(Dataset):
    """클라이언트별 분포 차이가 있는 unlabeled 합성 데이터."""
...
def main():
    ...
    client_datasets = [
        SyntheticUnlabeled(n=400, shift=+0.5, seed=1),
        SyntheticUnlabeled(n=400, shift=-0.5, seed=2),
        SyntheticUnlabeled(n=400, shift=+1.0, seed=3),
    ]

    server_loader = DataLoader(labeled_ds, batch_size=32, shuffle=True, collate_fn=labeled_collate)

    # 모델은 클라이언트별 1개, 서버 1개, global 1개를 만든다.
    def make_model():
